...
class TS2VecAR: # Edit: Rename model.
    ...
    def fit(self, train_data, n_epochs=None, n_iters=None, verbose=False):
        ...
        if n_iters is None and n_epochs is None:
            n_iters = 200 if train_data.size <= 100000 else 600 # 600  # default param for n_iters
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                ts_l = x.size(1)
                crop_l = np.random.randint(low=max(2 ** (self.temporal_unit + 1), self.time_steps + 1), high=ts_l+1) # Edited, but removed max(2 ** (self.temporal_unit + 1), 25) again
